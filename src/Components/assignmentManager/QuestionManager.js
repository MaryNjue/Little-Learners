...
import { Plus, Edit, Trash2, ArrowLeft, X, FileText, CheckCircle, Circle } from 'lucide-react';
import '../../App.css'; 
...
const QuestionFormModal = ({ isOpen, onClose, onSaved, assignmentId, initialQuestion }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                // Keys match Kotlin DTO: assignmentId, questionText, options, correctAnswer
            ...
            res = await fetch(`${API_BASE_URL}/api/questions`, {
                method: 'POST',
                // Explicitly set Content-Type for correct JSON parsing on the backend
        ...
        <div className="modal-overlay">
            <div className="modal-content">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <p className="text-xs text-gray-500 mt-2">Click the circle next to an option to mark it as the **Correct Answer**.</p>
...
const QuestionManager = ({ assignment, onBackToAssignments }) => {
    ...
    const fetchQuestions = async () => {
        ...
        try {
            ...
            const mappedData = data.map(q => ({
                ...q,
                // Backend returns options as a CSV string (q.options), so we must split it back into an array
                options: (q.options || '').split(',').filter(opt => opt.trim() !== '')
            }));
            // -------------------------
    ...
    if (loading) {
        return <div className="p-6 text-center text-lg text-blue-600">Loading questions...</div>;
    }
                                ...
                                {questions.map((q, index) => (
                                    <tr key={q.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-3">
                                            {/* Edit button disabled as PUT endpoint is not defined in QuestionController.kt */}
                                            {/* <button onClick={() => handleEditQuestion(q)} className="table-action-button edit" title="Edit Question"><Edit size={18} /></button> */}
                                            <button onClick={() => handleDeleteQuestion(q.id)} className="table-action-button delete" title="Delete Question"><Trash2 size={18} /></button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
